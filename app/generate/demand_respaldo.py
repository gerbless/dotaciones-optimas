...
def redefines_demand_in_skillful_blocks(brand_skillable_bloks, trx_high_demand_weeks):
    ...
    #i(filter_blocks,'datos/filter_blocks.csv')
    ...
    #i(redefines_demand,'datos/redefines_demand.csv')
    return redefines_demand

#setea min y max. min = 2. max = npos
def set_min_and_max_igual_two(redefines_demand:pd, location_codes: object, mirror: bool):
    switch_code_local = location_codes.location_mirror_code if  mirror is True  else location_codes.code
    query_pos = get_posts(id_location=switch_code_local).fetchone()

    if query_pos is None:
        npos = sqldf(''' select '{}' as local, 0 as tipo_caja '''.format(location_codes.code))
    else:
        obj = {'local':[location_codes.code],'tipo_caja':[query_pos[1]]}
    ...
    #i(redefines_demand_two,'datos/redefines_demand_two.csv')
...
#deferminar si el local cuenta con las transacciones pertinentes
...
def demand(id_process:int, location_codes: object, dates_search: dict):
    ...
    hours_preopening_and_postclosing = filter_according_to_preopening_and_postclosing(id_location=location_codes.code)
    ...
    redefines_demand_two = set_min_and_max_igual_two(redefines_demand=redefines_demand, location_codes=location_codes, mirror=mirror)
    ...
    #i(result_demand,'./datos/result/result_demand_{}.csv'.format(location_codes.code))

    return formatDemandJson, blocks_horus
